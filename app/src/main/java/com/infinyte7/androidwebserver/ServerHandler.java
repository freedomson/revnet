...
class ServerHandler extends Thread {
    ...
    private InputStream assetDocument;
    ...
    public void run() {
        ...
        corsEnabled = prefs.getBoolean("prefAllowCors", true);
    ...
    private void send(String text) {
        ...
        // if cors are enabled
        if(corsEnabled) {
            header = context.getString(R.string.header_cors,
    ...
    private void showHtml(String document, String[] ranges) {
        ...
        String documentFinal = "";
        if(document.charAt(document.length()-1) == '/') {
            documentFinal = "html/index.html";
        } else {
            documentFinal = "html/" + document;
        }

        boolean exists = AssetHelper.assetExists(context, documentFinal);
        Log.i("Retrieved document asset", documentFinal);
        Log.i("Retrieved document exist", String.valueOf(exists));

        document = documentRoot + document;
        document = document.replaceAll("[/]+","/");
        Log.i("Retrieved document file", document);
        ...
        try {
            ...
            if (rc == 200) {
                in = new BufferedInputStream(assetDocument);
                rcStr = context.getString(R.string.rc200);
                // contType = getMIMETypeForDocument(document).get(0);
                contType = AssetHelper.getMimeTypeForAsset(context, document);
            } else if (rc == -2) {
            ...
            am = context.getAssets();
            in = new BufferedInputStream(am.open(documentFinal));
            fileSize = (long) in.available();
            fileModified = DF.format(new File("file:///android_asset/" + documentFinal).lastModified()) + " GMT";
            contType = AssetHelper.getMimeTypeForAsset(context,documentFinal);
